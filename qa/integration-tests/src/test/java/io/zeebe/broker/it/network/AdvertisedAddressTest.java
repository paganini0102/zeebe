...
public class AdvertisedAddressTest {
  ...
  public void setup() {
    ...
    containers =
        IntStream.range(0, CLUSTER_SIZE)
            .mapToObj(i -> new ZeebeContainer("camunda/zeebe:current-test"))
            .collect(Collectors.toList());

    gateway = new ZeebeGatewayContainer("camunda/zeebe:current-test");
  ...
  private void configureBrokerContainer(final int index, final List<ZeebeContainer> brokers) {
    ...
    broker
        ...
        // propagated faster
        .withEnv("ZEEBE_BROKER_CLUSTER_MEMBERSHIP_SYNCINTERVAL", "1s");
  }
